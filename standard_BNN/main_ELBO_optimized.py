...
from bayesian_utils import DKL_gaussian_optimized, sampler_optimized
		...
		for e in range(ep):
			...
			for batch_idx,(x,t) in enumerate(trainLoader):
				...
				if dkl_after_epoch < total_epoch:#warm up
					_DKL_vae = 1*DKL_gaussian_optimized(q_m,q_logv,torch_zero,torch_zero) 
				...
				for i in range(MC_samples):
					sampled_params=sampler_optimized([q_m,q_logv],monte_carlo_sampling_list)#sample params from the variational distribution
			...
			for x,t in testLoader: #this is validation accuracy. We estimate this accuracy only with one monte carlo from the predictive distribution
				...
				q_m,q_logv=variatonal.forward()
				sampled_params=sampler_optimized([q_m,q_logv],monte_carlo_sampling_list)
				prediction=reconstruction.forward(x,sampled_params)
				_,index=torch.max(prediction,1)
			...
			if acc==save_after_epoch:
				model_dir=directory+str(t_eps)+'/models/'
