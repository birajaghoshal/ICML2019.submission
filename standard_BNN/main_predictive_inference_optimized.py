...
from VariatonalModels import VAE_reconstruction_optimized as VAE_reconstruction
from VariatonalModels import VAE_variational_optimized as VAE_variational
from calibration_utils import average_confidence,accuracy,accuracy_per_bin,average_confidence_per_bin,compute_ECE
from bayesian_utils import sampler_optimized


def parse_args():
	parser = argparse.ArgumentParser(description='Predictive Distribution  Enjoy!')
...
if __name__=='__main__':
	...
	monte_carlo_sampling_list=list()
	for i in range(len(variatonal.net)-1):
		l1=variatonal.net[i]
		l2=variatonal.net[i+1]
		monte_carlo_sampling_list.extend([torch.zeros(l1,l2).cuda(),torch.zeros(l2,).cuda()])
	########
	...
	if valid_test=='validation':
		...
		for i in range(1,MC_samples+1,1):
			...
			llh_mc=0.0
			parameters=sampler_optimized([q_m,q_logv],monte_carlo_sampling_list)
		...
		print "Best values is for diff {} with sample {} for ECE {} bins {} with sample {}".format(diff_best,sample_diff,n_bins,ECE_best,sample_ECE)
		...
		After calibration
		'''
		avg_confidence_afterBNN=average_confidence(predicted_afterBNN.numpy(),apply_softmax=False)
		acc_afterBNN=accuracy(predicted_afterBNN.numpy(),true_test.numpy(),apply_softmax=False)
		...
		ece_afterBNN=compute_ECE(accuracy_bin,conf_bin,samples_per_bin)

		print "ECE for {} bins without calibration getting {} with average confidence {} and accuraccy {}".format(n_bins,ece_before_BNN_test,avg_confidence_before_BNN_test,acc_before_BNN_test)
		print "ECE for {} bins and MC samples {} CALIBRATED getting {} with average confidence {} and accuraccy {}".format(n_bins,MC_samples,ece_afterBNN*100,avg_confidence_afterBNN*100,acc_afterBNN*100)
